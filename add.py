# -*- coding: utf-8 -*-
# add.py (Versão com atalho Ctrl+X corrigido)
...
def injetar_ankifant_no_addcards(add_cards: AddCards):
    if not add_cards or not add_cards.editor or not add_cards.editor.web:
        return

    editor_web = add_cards.editor.web
    config.active_webviews.add(editor_web)

    texts_json = json.dumps(TEXTS)
    pos_config_data = config.load_add_config()
    ...
    try:
        ...
        print(f"[Ankifant] ERRO AO LER IMAGENS (AddCards): {e}")
        return
    image_uris_json = json.dumps(image_uris)

    js_script = f"""
    (function() {{
        if (document.getElementById('ankifant-container-add')) return;
        ...
        function saveConfig() {{
            pycmd('ankifant_save_add_config:' + JSON.stringify({{pos: config.pos, visible: config.visible}}));
        }}
        ...
        function setLanguageExternal(lang) {{
            ...
            if (menuOpen) {{
                const oldMenu = document.getElementById('ankifant-menu-add');
        ...
        container.id = 'ankifant-container-add';
        ...
        ankifantImg.onclick = function() {{
            ...
            setTimeout(() => {{
                document.body.addEventListener('click', () => {{
                    if(document.getElementById('ankifant-menu-add')) {{
                        document.getElementById('ankifant-menu-add').remove();
                    }}
                    menuOpen = false;
                }}, {{ once: true }});
            }}, 0);
        ...
        function createMenu() {{
            ...
            const menuItems = [
                {{ key: 'add_menu_tips', contentKey: 'add_card_tips_content', titleKey: 'add_card_tips_title' }},
            ];
            ...
            menuItems.forEach(item => {{
                ...
                menuItem.onclick = (e) => {{ 
                    e.stopPropagation(); 
                    showDialog(texts[currentLang][item.titleKey], texts[currentLang][item.contentKey]);
                    menu.remove(); 
                    menuOpen = false; 
                }};
