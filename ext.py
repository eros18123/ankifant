# -*- coding: utf-8 -*-
# ext.py (Versão com menu corrigido e contextual)
...
def show_beautiful_dialog(title, content, parent):
    ...
    msg_box.setText(f"<h2 style='color: white; border-bottom: 1px solid #666; padding-bottom: 10px;'>{title}</h2><p>{content}</p>")
    msg_box.addButton("OK", QMessageBox.ButtonRole.AcceptRole)
    msg_box.exec()

def injetar_ankifant_nos_addons(addons_dialog):
    global ankifant_widget_instance

    if ankifant_widget_instance:
        try:
            ankifant_widget_instance.deleteLater()
        except RuntimeError:
            pass
    
    ankifant_widget_instance = QWidget(addons_dialog)
    widget = ankifant_widget_instance

    widget.setAttribute(Qt.WidgetAttribute.WA_TranslucentBackground, True)
    widget.setStyleSheet("background: transparent;")

    ext_conf = config.load_ext_config()
    ...
    def show_menu(pos):
        ...
        # Menu simplificado e corrigido para usar chaves de texto dedicadas
        action_info = QAction(texts["addons_menu_info"], widget)
        action_info.triggered.connect(
            lambda: show_beautiful_dialog(
                texts["addons_menu_info"], 
                texts["addons_info_alert"], 
                addons_dialog
            )
        )
    ...
    def mouseMoveEvent(event):
        ...
        parent_rect = addons_dialog.rect()
    ...
    def mouseReleaseEvent(event):
        if widget._dragging:
            ext_conf["pos"] = {"x": widget.pos().x(), "y": widget.pos().y()}
            config.save_ext_config(ext_conf)
        else:
    ...
    def toggle_visibility():
        ...
        ext_conf["visible"] = is_visible
        config.save_ext_config(ext_conf)

    shortcut = QShortcut(QKeySequence("Ctrl+X"), addons_dialog)
    ...
    def cleanup():
        ...
        try:
            ...
            pass
        ankifant_widget_instance = None
    addons_dialog.finished.connect(cleanup)

    pos = ext_conf.get("pos", {"x": 100, "y": 100})
    widget.move(pos["x"], pos["y"])
    
    if ext_conf.get("visible", True):
        widget.show()
